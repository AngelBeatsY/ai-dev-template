...
排除 `*-template.md`;design/assets/ 不检查(资产命名由 5.1 节资产清单约束)。脚本头部用法注释的检查项清单同步登记第 7 项。
...
- [ ] verify-docs 命名检查是否覆盖 design/assets/ 下的 md 资产 → 初步结论:不覆盖,资产命名由 5.1 节资产清单约束(待评审确认)
- [ ] structure.md 第 5 节决策树「页面规范、设计系统」类文档(非挂靠单 spec 的持久参考)是否属于 design/ 流水产物 → 待评审
