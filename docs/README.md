...
**维护规则**:创建流水产物时占号加行(与文件创建同一 commit);任何状态变化或文件修订时,同步文件头与注册表行(状态、更新日期,双面同语义,由修改该文件的会话同一 commit 执行——谁改谁刷,义务见 [conventions/workflow.md](conventions/workflow.md) 6.1);PR 合入时核对一致性。spec 行 MUST 填「来源」列(与文件头「来源」字段一致,受 verify-docs 检查 12 判据④ 校验;只填需求侧来源 BRIEF/RFC/无,多 spec 同源时重复填写;取代关系由 Superseded 状态与文件头互链承载,不入本列)—— 跨流关联以本列为权威,编号不承载关联语义。列状态取值见下方「生命周期」。
