...
摘要:所有改动先分级 —— 小改直接进入实现;常规级先写 brief 再写 spec 与 tasks;重大级先走 RFC,通过后回到常规流程;任意阶段产生的架构决策随手记 ADR。
...
2. **分级声明协议。** AI 开工前 MUST 向用户输出「本次改动判断为 X 级,依据是 …」并等待确认;AI 只有建议权,确认权在人。
3. **小改累计升级。** 同一目标下连续多个小改,累计触及公共接口或累计影响 > 10 个文件时,MUST 停下来补 brief / spec,防止用小改通道走私常规变更。
...
- **评审清单**:
  ...
  - 测试计划覆盖每条 FR。
- **通过标准**:评审清单全绿,状态转 Active,方可生成 tasks。
...
- 完全取代(新 spec 覆盖旧行为全部):旧 spec 转 Superseded,注册表与文件头互相标注。
...
- **拆分原则**:
  ...
  - 内置三条固定收尾任务,不可删除:更新相关文档、更新 `docs/README.md` 注册表、对照 spec 逐条自检。

**立项交付**:立项会话结束前自查 —— brief 已 Accepted、spec 已 Active、tasks 就绪、注册表与 STATUS 已登记、开工提示词已输出;交付物 MUST 通过施工会话的开工核对(见第 3.4 节 DoR,即以 DoR 为验收标准)。
...
- [ ] brief 状态 Accepted,spec 状态 Active(评审清单已全绿)。
- [ ] spec 每条 FR/ER 可判对错;现状盘点有证据(文件行号或 `research/` 落档结论)。
...
- **AI 执行协议**:
  ...
  - 每个任务/阶段完成后在 tasks 回填**交付记录**(commit hash、测试与 lint 结果及与开工基线的比较、交接要点);交付记录与代码 MUST 同一 commit,禁止代码先走、文档后补。
...
- **双检制**:AI 自检 + 人工评审,两者都不可省。
- **AI 自检**:实现完成后,AI 按 PR 模板的「提交者自检」清单逐项核对并报告结果。
...
- 合同时 MUST 完成:tasks 全部勾选;spec 状态转 Implemented 并更新注册表;由 RFC 派生的改动回填 RFC 决议记录中的 spec 编号。
...
- **append-only 铁律**:Accepted 后正文 MUST NOT 修改,只能被新 ADR 取代;允许更新的只有状态行与 superseded-by 链接。
...
tasks 模板已内置注册表更新任务;PR 自检清单含注册表核对项。漏更新注册表视为未完成。
