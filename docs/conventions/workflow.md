...
- **三层检查**:实现方自检 + 独立验收(常规级及以上,小改豁免)+ 人工评审,逐层不可省。
...
- 合同时 MUST 完成:tasks 全部勾选;spec 状态转 Implemented 并更新注册表;由 RFC 派生的改动回填 RFC 决议记录中的 spec 编号。收口回填以 PR 尾部 docs commit 随分支合入(AI 执行合并与人在平台侧执行均适用);PR 已合入后发现回填遗漏,以新分支 docs PR 补救(小改流程)。回填角色归属见第 7 节,与合并执行者解耦。
...
| 独立验收适用范围 | 常规级及以上(小改豁免) | 团队项目 MAY 放宽至仅重大级(人工评审在位时);纯个人项目豁免人工评审的,常规级以上独立验收 MUST 保留,不在放宽范围 |
